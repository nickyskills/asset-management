                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.services.update", [$service->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('cartegories') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="cartegories[]" id="cartegories" multiple>
                                @foreach($cartegories as $id => $cartegory)
                                    <option value="{{ $id }}" {{ (in_array($id, old('cartegories', [])) || $service->cartegories->contains($id)) ? 'selected' : '' }}>{{ $cartegory }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="status" id="status" value="{{ old('status', $service->status) }}">
                        ...
                        <div class="form-group {{ $errors->has('priority') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control" name="priority" id="priority">
                                ...
                                @foreach(App\Service::PRIORITY_SELECT as $key => $label)
                                    <option value="{{ $key }}" {{ old('priority', $service->priority) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('usage') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="usage" id="usage">{!! old('usage', $service->usage) !!}</textarea>
                            @if($errors->has('usage'))
