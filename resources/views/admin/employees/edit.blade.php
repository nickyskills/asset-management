                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.employees.update", [$employee->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('employee_number') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="employee_number" id="employee_number" value="{{ old('employee_number', $employee->employee_number) }}">
                        ...
                        <div class="form-group {{ $errors->has('middle_name') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="middle_name" id="middle_name" value="{{ old('middle_name', $employee->middle_name) }}">
                        ...
                        <div class="form-group {{ $errors->has('address') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="address" id="address" value="{{ old('address', $employee->address) }}">
                        ...
                        <div class="form-group {{ $errors->has('branch') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="branch_id" id="branch_id">
                                @foreach($branches as $id => $branch)
                                    <option value="{{ $id }}" {{ ($employee->branch ? $employee->branch->id : old('branch_id')) == $id ? 'selected' : '' }}>{{ $branch }}</option>
                                @endforeach
                            </select>
